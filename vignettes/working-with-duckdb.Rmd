...
The PSInet database is organized around **7 core tables** containing information about data collection, experimental design, and authorship, and **5 measurement tables** containing the actual measurements. Additionally, one core and each measurement table has an associated **quality control flag table**, and there is a table of SAPFLUXNET IDs for cross-compatibility. The `meta` table provides authorship metadata (author names and ORCID IDs) for each dataset, joining via `dataset_name`.
...
The database includes five main types of measurement tables:
...
Each core table (`plant`) and all five measurement tables have an associated flag table (e.g., `chamber_wp_flag`, `auto_wp_flag`, etc.). Flag tables are identical in dimensions to their corresponding data tables and contain only logical TRUE/FALSE values. Flag tables also include a `dataset_name` column, enabling direct joins with other tables. These values indicate whether each entry passes our quality control checks, including range checks that can be found in the [PSInetDB data schema documentation](https://github.com/PSInetRCN/PSInetDB/blob/main/docs/schema/data_schema.md).
...
core_tables <- c("study_site", "plot", "plant", "treatment", "data_description", "addtl_data", "meta")
