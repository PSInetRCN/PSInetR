...
data_dir <- get_data_dir()  # Get default data directory
db_path <- file.path(data_dir, "psi_data.duckdb")
...
# Get water potential data for a specific plant
wp_data <- tbl(con, "press_chamb_wp") %>%
  filter(individual_id == "PINE_01") %>%
  collect()

# Plot water potential over time
wp_data %>%
  mutate(date = as.Date(as.character(date), format = "%Y%m%d")) %>%
  ggplot(aes(x = date, y = water_potential_mean)) +
  geom_point() +
  geom_errorbar(aes(ymin = water_potential_mean - water_potential_sd,
                    ymax = water_potential_mean + water_potential_sd)) +
  labs(title = "Plant Water Potential Over Time",
       x = "Date",
...
# Join water potential and VPD data by date
combined_data <- wp_data %>%
  inner_join(env_data, by = c("dataset_name", "date")) %>%
...
wp_data <- wp_data %>%
  mutate(
    time_of_day = case_when(
      grepl("dawn|predawn|pre-dawn|early", time, ignore.case = TRUE) ~ "Predawn",
      grepl("mid|noon|after", time, ignore.case = TRUE) ~ "Midday",
      TRUE ~ "Other"
    )
  ) %>%
