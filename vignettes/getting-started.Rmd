...
7. **authorship**: Contains authorship metadata for each dataset, including submitting and corresponding author names and ORCID IDs. Joins to other tables via `dataset_name`. Note that some datasets (~26 of 268) do not yet have authorship entries.
...
2. **auto_wp_sensor**: Sensor details associated with automated measurements.
...
authorship → (joins to any table via dataset_name — authorship metadata)
```
